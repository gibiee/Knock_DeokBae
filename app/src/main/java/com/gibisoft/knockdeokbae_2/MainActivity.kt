...
class MainActivity : AppCompatActivity() {
    ...
    fun onFragmentChange(str : String) {
        when(str) {
            ...
            "게임 플레이_easy" -> supportFragmentManager.beginTransaction().replace(R.id.container, GamePlay(50)).commit()
            "게임 플레이_normal" -> supportFragmentManager.beginTransaction().replace(R.id.container, GamePlay(80)).commit()
            "게임 플레이_hard" -> supportFragmentManager.beginTransaction().replace(R.id.container, GamePlay(110)).commit()
